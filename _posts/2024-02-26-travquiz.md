...
const questions = [
  ...
  },
  {
    ...
    function calculateDestination() {
        ...
        fetchMatchingDestinations(recommendedDestination);
    }

    function fetchMatchingDestinations(recommendedDestination) {
        fetch(`/api/travel?travel_type=${recommendedDestination}`)
            .then(response => response.json())
            .then(data => {
                displayMatchingDestinations(data);
            })
            .catch(error => {
    ...
    function displayMatchingDestinations(destinations) {
        const matchingDestinationsDiv = document.createElement('div');
        matchingDestinationsDiv.innerHTML = `<h2>Matching Destinations</h2><ul>${destinations.map(destination => `<li>${destination.destination}</li>`).join('')}</ul>`;
        document.body.appendChild(matchingDestinationsDiv);
    }
</script>
